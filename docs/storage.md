...
- Findings (agent.md): `n-plus-one` is `GROUP BY trace_id, query_id HAVING COUNT(*) >= 5` over the database spans of the window, attributed to the entry span by the same parent-chain walk the query callers use; `dbCallsPerRequest` and `dbMsPerRequest` of an endpoint join the endpoint's entry spans with the database spans of the same trace and service; `slow-job` is the same aggregation over `span WHERE parent_span_id IS NULL AND kind = 'INTERNAL'` grouped by `(service, name)`, with the database work joined the same way; the other kinds are the endpoint, query and error aggregations above, filtered by the thresholds.
  The parent-chain walk reads the window's spans once per answer; for one service it reads only the traces that have a span of that service in the window, which is where every entry span its spans can reach lies.
- The traces that contain a group (a finding's evidence and sample, the slowest and newest traces of the endpoint, query and error pages): `SELECT * FROM trace t WHERE t.start_ms BETWEEN ? AND ? AND t.trace_id IN (SELECT s.trace_id FROM span s WHERE s.<group> = ? AND s.start_ms BETWEEN ? AND ?)`.
  The matching span starts in the window too, so the subquery is a range of the group's `(…, start_ms)` index and costs the window's share of the group, never its whole history.
...
- The acknowledgements and resolutions of a page of findings: `SELECT * FROM ack WHERE finding_id IN (…)`, one statement for the page. A finding's state (agent.md) reads the two newest `start` marks of its service, `SELECT at_ms FROM mark WHERE name = 'start' AND service = ? AND at_ms <= ? ORDER BY at_ms DESC LIMIT 2`, and runs the rules of the page's kinds again over the run between them, for ids alone, keeping what they found in memory for as long as the server runs (agent.md, "State").
