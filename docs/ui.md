...
- The recording (`assets/js/dev/replay.js`, loaded when `<html data-snapshot="…">` or `?snapshot=<dir>` names a snapshot directory) answers every `GET /api/…` from JSON files that `scripts/demo-site.mjs capture` fetched from a Spider Sense loaded with the recorded rows (design.md, "The published demo"), keyed by path and query without `from`/`to`; a request whose filters were not recorded falls back to the nearest recorded list, dropping one filter at a time, and logs the substitution. It freezes `Date.now()` at the end of the window, makes the default top-bar range the length of the recording (the `15m` preset becomes "The recording (5 min)", so every chart covers the recorded minutes and none before them), answers `POST` and `DELETE` with 405, replaces `EventSource` with one that never emits, and puts one line above the top bar saying when the recording was made, with a link to the DoltHub database that holds its rows (`manifest.source`). `scripts/demo-site.mjs assemble` copies `public/` and the answers into one directory with `data-snapshot` set on `index.html`, which is the static demo the manual publishes at `/demo` (design.md).
