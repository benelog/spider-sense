...
- Top bar: page title on the left; on the right the **Mark** button, the **service filter** (all services or one; applies to every page that takes `service`), the **time range** (`5m`, `15m`, `1h`, `6h`, `all`; `all` means `from = /api/status.oldest.span`), the **editor** setting, and the **Live** toggle.
- **Editor**: a select, **IntelliJ IDEA** (the default) or **VS Code**, naming what a code frame link opens ([Code frames](#code-frames)); the choice is remembered in `localStorage` (every access wrapped in `try`, so a browser that refuses storage keeps the default) and applied at once to every link on the page.
...
- The sidebar foot shows the mode, the port, the version, and a "how to send data" link that opens the snippet dialog. When `/api/status.storage.droppedSpans` is above zero, or a `stats` event raised it, the foot adds a line `dropping spans` in `--warn` with the count as its title, kept for a minute after the last increase, so a load test that crossed `spidersense.ingest.max-spans-per-second` is seen as partial data rather than as a quiet application.
...
A table: `#`, severity (the dot and the word), state chip, kind chip, service chip, title, and the impact number the kind is ranked by in a right-aligned column (a `regression` shows its `originalKind`'s, `count` for `error` and `log-error`, `medianRepeats × affected` shown as `42 × 3` for `n-plus-one` and `n-plus-one-http`, `totalMs` for `slow-query`, `slow-endpoint`, `slow-job` and `slow-external`, `pendingMax` for `pool-exhausted`, `worstMs` for `gc-pause`, `ratioMax` as a percentage for `heap-pressure`, `last` minus `first` for `thread-growth`).
...
`hotSpans` and `breakdown` (agent.md, "Where the time went"), which a `slow-endpoint` and a `slow-job` carry, are two rows of the numbers grid spanning its full width rather than cells in it: **hot spans** as a short list, the summary in monospace with `<selfMs> · <share> · ×<count>` dimmed beside it, and **breakdown** as the same stacked bar the endpoint page draws with the four shares written beside it.
The `code` frames are [code frames](#code-frames): each is a link into the editor with its five source lines under it when it resolves, and plain monospace text when it does not.
...
The expanded evidence of any finding ends with a **Resolve** button (a small dialog for the optional note, then `POST /api/findings/{id}/resolve`) and an **Acknowledge** button (the same dialog, then `POST /api/findings/{id}/ack`); when acknowledged, it ends with the note and an **Unacknowledge** button (`DELETE …/ack`) instead, and when resolved, a regression included, with the resolution's instant and note and a **Reopen** button (`DELETE …/resolve`).
The Overview's Findings panel asks with `hideAcked=true`, so the top five are never acknowledged ones or resolved ones that have not come back, and regressions come first.
...
The header also carries the **Time breakdown**: `breakdown` from `/api/endpoints/{endpointId}` (api.md) as one horizontal stacked bar, 120×5 px like the status bar beside it, the four shares in a fixed order and a fixed colour each — `db` in `--accent`, `http` in `--series-8`, `internal` in `--warn`, `self` in `--silk` — each segment sized by its share, with the share and the bucket in the bar's title and the largest bucket named in words under the caption, so the colour is not the only carrier. Nothing is drawn when `breakdown` is `{}`.
...
A code frame the findings page or the error page shows asks `GET /api/source?frame=…` (api.md) once, and an answer is reused for ten seconds so a Live refresh does not ask again at once.
A frame that resolves becomes a link, `idea://open?file=<absolute path>&line=<n>` or `vscode://file<absolute path>:<n>` as the top bar's **Editor** setting says, with the path and line as its title, and the five lines around its line follow under it in monospace, numbered, the frame's own line on `--accent-soft`.
