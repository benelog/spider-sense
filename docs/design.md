...
The recording is the rows themselves, kept in a DoltHub database, <https://www.dolthub.com/repositories/benelog/spider-sense-demo>, with the tables of [storage.md](storage.md) as they are (`service`, `span`, `trace`, `log`, `metric`, `metric_series`, `metric_point`, `tingle`, `mark`, `ack`, `meta`), so a trace of the demo can be queried there as SQL, and each recording is a commit of that database rather than of this repository.
`scripts/demo-site.sh record` runs `demo-shared.sh` for `MINUTES` (5), marks `before` and `after` a third and two thirds of the way through so the compare page has two windows, and then `scripts/demo-site.mjs export` copies every row since the launch out of the H2 file into one CSV per table (H2's `CSVWRITE`, driven through `org.h2.tools.Shell` from the nested server jar, so the copy needs nothing but the jar), with the home directory anonymised as agent.md's examples are and the window the page shows written into `meta` as `recording.from` and `recording.to`; `push` uploads each CSV through DoltHub's import API, which lands it on a branch of its own, and merges that branch into `main`.
`scripts/demo-site.sh assemble` is the other direction, and what `npm run docs` runs: `pull` downloads the tables as CSV, a fresh standalone Spider Sense is started on `build/demo-data/sense` with retention switched off, `load` inserts the CSVs into it (`CSVREAD`, after checking that the recording's `schema_version` is the server's), `capture` asks it every question the UI asks over the recorded window (the lists unfiltered and per service, every endpoint, query, error and trace they mention, every metric series, every pair of marks for `compare`) and writes the answers under `build/demo-data/answers/` as JSON files named by a hash of their request, with `manifest.json` mapping request to file and carrying the window and the DoltHub commit, and `assemble` copies the UI and the answers into `build/demo-site/` and marks `index.html` with `data-snapshot`, which makes `app.js` load `assets/js/dev/replay.js` instead of talking to a server ([ui.md](ui.md)).
The page is therefore the same `Queries` the UI and the CLI use, run once at build time over the recorded rows, never a second implementation of them in the browser; the Docs workflow builds the jar for it, and nothing of the recording is committed to this repository.
Re-recording is `scripts/demo-site.sh record` with `DOLTHUB_TOKEN` set (a writer's API token of the database, kept in `.envrc`), followed by a run of the Docs workflow.
