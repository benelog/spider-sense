...
`scripts/demo-shared.sh` builds the jar and starts one standalone Spider Sense (4000) that the four apps and the load generator forward to, which is where a trace crossing two services and one `findings` over every service show up in one place; `scripts/demo.sh` is the other layout, each app in agent mode hosting its own Spider Sense UI (bookstore 4000, orders 4001, warehouse 4002, worker 4003) with no extra process. Both take `--no-build` and `RPS`, log every process under `build/demo-logs/`, and print the CLI line to run next. The README and the manual's examples chapter carry a prompt that hands the running demo to an AI agent: `/spider-sense` (`$spider-sense` in Codex) and a question, with no command in it, asking for the three biggest problems, the line under `examples/` behind each, and a proposed fix before any change. `skills/` is linked from `.claude/skills/` and `.agents/skills/`, where Claude Code and Codex look for a project's skills, so an agent started in the checkout has the skill without `init`.
...
`scripts/agent-demo.sh record <claude|codex> <en|ko>` does what a user of the prompt does: it starts `scripts/demo-shared.sh --no-build`, lets it take traffic for `WARMUP_MINUTES` (3), and only then runs the agent headless in this checkout on that prompt (the Korean one is its translation), `claude -p --output-format stream-json` or `codex exec --json`.
...
Claude Code runs with the project's settings only and none of the account's claude.ai connectors or the user's auto memory (plus `"language": "korean"` for a Korean session, since the user's own setting is what makes it answer in Korean), no edit tool, and one appended line of system prompt saying that the session ends with the turn, since `-p` does not wait for a wait it left in the background; Codex runs in its `workspace-write` sandbox with the network open, since the CLI asks the running Spider Sense over HTTP, at `model_reasoning_effort` `CODEX_EFFORT` (`medium`), and the script writes down its version and model for the push.
The tree is clean when a session is recorded, so what the agent reads of it is the commit, and the session has a database of its own under `build/agent-demo/db/` (`SPIDERSENSE_CONFIG` names a file with `spidersense.db`), so its findings cover that run and not what earlier ones left in `~/db/spider-sense/sense`.
