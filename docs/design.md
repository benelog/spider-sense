...
The fourth thing the extension does is the one that needs a `Connection`, which no span processor has: it reads the index catalog of the tables a slow statement touches, so a `slow-query` or `n-plus-one` finding can say which of the columns the statement filters on no index serves ([agent.md](agent.md#the-schema-block)).
For that it carries one `InstrumentationModule` of its own, `spider-sense-schema`, registered through `META-INF/services/io.opentelemetry.javaagent.extension.instrumentation.InstrumentationModule`: an advice on the `execute`, `executeQuery`, `executeUpdate` and `executeLargeUpdate` methods of every `java.sql.Statement` implementation, which measures the call and, when it threw nothing and either took at least `spidersense.slow.query.ms` (the same threshold, read the same way) or is the fifth execution of the same SQL text within the current trace, hands the statement to `IndexCatalog`.
The repeat is counted as the span processor counts it: per thread, keyed by the trace id of the current span and reset when another trace's statement runs on that thread, 256 distinct statements at most; it is what gives an `n-plus-one` finding its block, since the individual queries of an N+1 are never slow.
...
The recording is a DoltHub database, <https://www.dolthub.com/repositories/benelog/spider-sense-demo>, with the tables of [storage.md](storage.md) as they are (`service`, `span`, `trace`, `log`, `metric`, `metric_series`, `metric_point`, `tingle`, `mark`, `ack`, `meta`) and one table more, `answer`: every answer the UI asks for, keyed by path and query without `from`/`to`, plus `/manifest` (the window, the services, the keys).
