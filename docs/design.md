...
Claude Code runs with the project's settings only and none of the account's claude.ai connectors or the user's auto memory (plus `"language": "korean"` for a Korean session, since the user's own setting is what makes it answer in Korean), no edit tool, and one appended line of system prompt saying that the session ends with the turn, since `-p` does not wait for a wait it left in the background; Codex runs in its `workspace-write` sandbox with the network open and `~/db/spider-sense` writable, since the demo needs both.
