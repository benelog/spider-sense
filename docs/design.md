...
It bites the repeated HTTP call hardest, because a client that completes the exchange off the calling thread ends every span of the run on a different worker — Spring's `JdkClientHttpRequestFactory` over the JDK `HttpClient` is one, and it is why `examples/spring-orders` produces an `n-plus-one-http` finding that names no line.
