...
The demo is also published as a static page, <https://spider-sense.benelog.net/demo>: the real UI over a recording of the shared demo, with no server behind it. `scripts/demo-site.sh` runs `demo-shared.sh` for `MINUTES` (5), marks `before` and `after` a third and two thirds of the way through so the compare page has two windows, and then `scripts/demo-site.mjs capture` asks the running Spider Sense every question the UI asks over that window (the lists unfiltered and per service, every endpoint, query, error and trace they mention, every metric series, every pair of marks for `compare`) and writes the answers under `demo/data/` as JSON files named by a hash of their request, with `manifest.json` mapping request to file and carrying the window; the home directory is anonymised as agent.md's examples are. `scripts/demo-site.mjs assemble` copies the UI and the data into `build/demo-site/` and marks `index.html` with `data-snapshot`, which makes `app.js` load `assets/js/dev/replay.js` instead of talking to a server ([ui.md](ui.md)). `demo/data/` is committed, so `npm run docs` assembles the page into `build/site/demo` without running Java, and the Docs workflow publishes it with the manual; re-recording is `scripts/demo-site.sh` followed by a commit of `demo/data/`.
