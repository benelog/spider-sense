...
The repository carries an agent skill at `skills/spider-sense/`: a `SKILL.md` an agent loads once a task touches Spider Sense, and reference files it reads only as the task needs them.

The skill teaches the loop itself rather than the commands alone: start the application under the agent, mark a moment, exercise it, read the findings, fix, compare, check (xref:agent-loop.adoc#the-loop[The Loop]).
An agent that has only the CLI's `help` knows every option and none of the method.
The skill is distilled from this manual, and the manual stays the source of truth.

== What is in it

`SKILL.md` carries what every task needs: what Spider Sense gives an agent that a dashboard does not, the loop as a script of commands, how to start each kind of application under the agent, how to read a finding, and the rules that keep an answer honest.
...
The last line names `skills/spider-sense/` of the https://github.com/{project-repo}[Spider Sense repository] instead of the project's own copy when `--no-skill` kept the skill from being installed.
...
`init` copies `SKILL.md` and `references/*.md` into `<dir>/.claude/skills/spider-sense/`, unless `--no-skill` is given.
It overwrites the files it owns and leaves anything else in that directory alone.
...
The second is `skipped skill (--no-skill)` when the skill was not installed.
...
== Installing the skill by hand

`init` is the normal way, and it installs for Claude Code.
Copy the directory yourself when the jar is not at hand, or when the target is another tool.

=== Claude Code

Claude Code reads a project's `.claude/skills/`, which is where `init` puts the skill.
Copy the directory there instead to get the same result, so that collaborators get the skill by cloning and install nothing of their own:
...
Copy `skills/spider-sense` into either one.
...
Nothing in the skill is new information.
Every statement in it is this manual, compressed for an agent's context window instead of a reader's screen.
A question the skill cannot settle is answered here, and a release that changes a command or a finding changes the skill alongside it.
