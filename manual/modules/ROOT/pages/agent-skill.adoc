...
The jar path above is the one `init` resolved, and every other line is written exactly as shown.
...
`init` prints one line per skill, each with the number of files it wrote.
...
`--mcp` adds a last line, `wrote .mcp.json (spider-sense over stdio)` or `updated .mcp.json (…)`, and writes the stdio MCP server into the project's `.mcp.json` (xref:mcp.adoc[MCP]).
