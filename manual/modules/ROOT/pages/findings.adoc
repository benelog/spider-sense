...
The block is `null` rather than wrong whenever the parse cannot vouch for it: a table the catalog has no row for (the application ran in standalone mode or without the extension, or no statement on that table has yet been slow or repeated five times in one trace), a statement whose tables the scanner does not find, or a column it cannot attribute to one table, such as an unqualified column in a join or the alias of a subquery.
