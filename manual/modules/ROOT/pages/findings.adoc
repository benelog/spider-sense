...
For `n-plus-one-http` it is there only when the client ends the call's span on the thread that made the call: the JDK `HttpClient` and the reactive clients complete the exchange on a worker thread, so the counter behind it never reaches five (xref:design.adoc#extension[The extension]).
...
Only the spans of the finding's own service count, the rule `dbShare` already uses: leaving the downstream server span out is what makes the wait on an outbound call land in `http` where the caller can see it.
...
`hotSpans` is every span but the top ones, grouped by its category and its name, summed, the largest `selfMs` first.
...
`self` is the top spans' own self time, which is the request's own code and the framework, and `internal` absorbs the `messaging` and `rpc` categories: the question the four answer is whether the time was spent waiting on the database, waiting on the network, or working.
It is the number to read first when `dbShare` is low, and the xref:pages.adoc#endpoints[endpoint page] draws the same four shares as one bar.
...
The severity, the impact and the `numbers` are `n-plus-one`'s, over the repeated call rather than the repeated statement; `statement` is `null` and `subject.target` names the host that was called.
...
The digits are what a loop varies, so replacing them is what makes three calls one call.
The status is not part of the call, because the same call can answer `200` for one item and `404` for the next, and the host keeps its port because the digit rule is applied to the path alone.
`code` is the call site of the fifth repeat, but only when the client ends the span on the thread that made the call; otherwise the finding names no line and the trace tree shows the loop instead.
...
* When neither is possible, make the calls concurrently, which shortens the request without removing the load.
...
* *Low `dbShare`* means the time is elsewhere.
  ...
  `hotSpan` names the one span that holds it in that one trace.
...
Low means the time is the job's own code, and <<time,`breakdown`>> and `hotSpans` say where it went before a trace has to be opened.
