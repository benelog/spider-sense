...
The task also sets `SPIDERSENSE_URL` to the base URL the `spiderSense` block implies, so the command asks the Spider Sense the application is sending to rather than the default port (xref:gradle-plugin.adoc#tasks[Tasks]).
