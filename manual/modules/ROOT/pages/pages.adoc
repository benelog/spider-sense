...
The Apdex tile reads `0.93`, and its title says the T it was computed with.
...
Beside it, the *Response summary* draws five vertical bars, one per response-time bucket and one for errors, with the count above each bar and the label beneath.
...
Under the statement of a `slow-query` or an `n-plus-one` comes the xref:findings.adoc#schema[schema block], in small monospace, the lines the CLI prints: one per table with the indexes it carries, `none` for a table that has none, then one for the columns the statement filters on, with the ones no index leads with named after `unindexed` and in the accent colour.
...
Clicking a header sorts by that column, and the default is total time.
...
Beside them is the *time breakdown*, one stacked bar in four fixed colours — `db` in the accent colour, `http` in teal, `internal` in the warning colour, `self` in the strand grey — over the endpoint's 20 slowest traces in the window, the same four shares a xref:findings.adoc#time[`slow-endpoint`] finding carries.
