...
xref:examples.adoc[The Examples] has `scripts/demo.sh` and `scripts/demo-shared.sh`, which are these modes in working form: two applications each with their own embedded Spider Sense, and the same two forwarding to one standalone.
