...
A `spider-sense.properties` in the project directory is read by the launcher as well, because the forked JVM's working directory is the project's, and `configFile` names another one.
...
When the block names none of them the variable stays unset, and the CLI's own default applies: what the `spidersense.*` properties the task gets imply, so a port in `configFile`, or in `spider-sense.properties`, is followed too.
...
It asks the Spider Sense the block implies, through `SPIDERSENSE_URL` as above or the CLI's own default, and prints the check's text rendering.
