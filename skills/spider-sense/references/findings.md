...
`code` is the call site of the fifth repeat, so it names the line that issues the call; the repeats are counted per trace, so an asynchronous client that ends each of them on a different worker is counted like any other.
