...
| `GET /api/orders/{id}/enriched` | Loads the order and calls `GET http://localhost:8081/api/books/{productId}` with `RestClient`, once per line | **N+1 over HTTP on purpose** and a **cross-service trace**: a seeded order has two to six lines, so most requests make the five calls that make it an `n-plus-one-http` finding; when the bookstore is down every `book` is `null` and the status is still 200 |
| `GET /api/orders/{id}/enriched-batch` | The same answer, asking `GET http://localhost:8081/api/books?ids=1,2,3` once | **the fix for the HTTP loop**, so the endpoint above can be measured against it with `compare`. It still lazy-loads every product, so the `n-plus-one` over `SELECT product` is on both: one fix at a time is what the loop is meant to show |
