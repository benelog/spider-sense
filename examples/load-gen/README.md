...
A plain Java 21 program that sends a steady, jittered trickle of requests to `silk-bookstore` (8081) and `spring-orders` (8082), so the Spider Sense dashboards fill up without anyone clicking.
...
At startup it polls `/api/health` on both apps for up to `--wait` seconds, printing `waiting for …` once per app, and then sends traffic regardless; an app that never comes up simply produces counted failures.
...
Weights are relative and add up to 109; the table lives in `Scenarios.java`.
...
The tests check that the weights add up, that every scenario builds an absolute URI against either base URL, that the checkout sequence substitutes the created order's id, and that the duration ring keeps only the last 1 000 samples and computes the right p95.
