...
The manual under `manual/modules/ROOT/pages/` is the specification: read the pages that cover a change before making it, and a change to behaviour is a change to them first.
The ones to know are `design.adoc` (architecture and decisions), `storage.adoc` (the H2 schema, writer, queries, retention), `api.adoc` (the JSON contract between server and UI), `ui.adoc` and `pages.adoc` (layout, look and feel, the pages), `findings.adoc`, `marks-and-compare.adoc`, `check.adoc`, `cli.adoc`, `mcp.adoc` and `agent-skill.adoc` (what AI agents use: findings, marks, compare, check, the text renderings, the CLI and the skill), and `gradle-plugin.adoc` and `maven.adoc` (the build tools).
...
| `spider-sense-gradle-plugin` | An included build (`pluginManagement.includeBuild`), not a subproject: the Gradle plugin `net.benelog.spidersense` that puts `-javaagent` on `bootRun`/`run`, the `spiderSense` block, and the `spiderSense`/`spiderSenseInit`/`spiderSenseCheck` tasks (`gradle-plugin.adoc`). Published to Maven Central with the jar. |
| `spider-sense-extension` | The OpenTelemetry agent extension: `CallSiteSpanProcessor`, which records `code.stacktrace` on a slow query, the fifth repeat of a statement, a slow outbound call and the fifth repeat of an HTTP call; `SchemaLookupSampler`, which drops the spans of its own catalog lookups; and two instrumentation modules, `spider-sense-schema`, whose advice hands a slow statement to `IndexCatalog` to emit the tables' indexes, and `spider-sense-spider-silk`, which names a Spider Silk request after its route. Compiled `compileOnly` against the SDK and the agent's extension API, nested as `spider-sense/extension.jar`. |
...
- **Storage is H2 under `~/db/spider-sense/`** (`storage.adoc`), opened with `AUTO_SERVER=TRUE` so several processes share it and the data outlives the monitored application. Plain JDBC, no ORM; nothing else keeps state.
- **Every agent-facing answer comes from the same `Queries` as the UI**, and its text rendering is deterministic over the window (`cli.adoc`); the CLI never renders on its own, it prints what the server or the in-process renderer produced.
...
- **The manual lives in `manual/`, as an Antora component, and is published at <https://spider-sense.benelog.net> by `.github/workflows/docs.yml`.**
  ...
  The manual is the one specification and the one user documentation; there is no separate spec to keep in step, so a change to behaviour and the page that describes it land in the same commit.
  The user-facing chapters come first in `nav.adoc`, and implementer-facing detail (the jar's layout, the extension, the schema, the JSON contract) goes into the Reference group.
...
java -jar spider-sense-agent/build/libs/spider-sense-<version>.jar findings --since=start   # the CLI (cli.adoc)
